...
Describe "Azure helper (live Azure Blob): LFS round-trip via git-lfs-object-store"
	...
	# assertions can name the exact container key without recomputing.
	make_fixture() {
		local out="$1"
	...
	Describe "push then clone"
		...
		It "uploads the object to <prefix>/lfs/<oid> and round-trips on clone"
			if live_engine_is_bundle; then
				assert_lfs_object_exists live_az_list "$CONTAINER" "$PREFIX" "$OID"
			fi
