...
// Re-export the cross-crate helpers so existing scenarios (and the
// `super::packchain_live::*` import in `zip_fault.rs`) continue to find
// `git_available`, `git`, `git_capture`, `make_seed_repo`, and
// `drive_in` here without referencing the lib's `test_util` path
// directly.
