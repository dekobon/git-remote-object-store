...
| `GIT_REMOTE_OBJECT_STORE_LOCK_TTL_SECONDS` | `60` | Per-ref lock TTL in seconds. Locks older than this are considered stale. Honored by the helper push path, `compact --lock-ttl-seconds`, `delete-branch`, and library consumers of `DoctorOpts::default()` (the `doctor --lock-ttl` CLI flag still always passes an explicit value — issue #178 will let the flag itself defer to this env var). | `src/protocol/push.rs` (`ENV_LOCK_TTL_SECONDS`) |
